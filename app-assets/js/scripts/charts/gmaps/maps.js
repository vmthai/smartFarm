...
var getLng = '100.49265657709634'; 
    ...
    function initialize() { // ฟังก์ชันแสดงแผนที่
        ...
        GGM.event.addListener(my_Marker, 'dragend', function() {
            ...
            parent.$("#lat_value").val(my_Point.lat());  // เอาค่า latitude ตัว marker แสดงใน textbox id=lat_value
            parent.$("#lon_value").val(my_Point.lng());  // เอาค่า longitude ตัว marker แสดงใน textbox id=lon_value 
            parent.$("#zoom_value").val(map.getZoom());  // เอาขนาด zoom ของแผนที่แสดงใน textbox id=zoom_valu   
            parent.$("#namePlace").val(''); // ลบข้อมูลการค้นหาออก       
    ...
    $(function(){
        ...
        var searchPlace=function(){ // ฟังก์ชัน สำหรับคันหาสถานที่ ชื่อ searchPlace
            var AddressSearch=parent.$("#namePlace").val();// เอาค่าจาก textbox ที่กรอกมาไว้ในตัวแปร
            if(geocoder){ // ตรวจสอบว่าถ้ามี Geocoder Object 
                geocoder.geocode({
                    ...
                    if(status == GGM.GeocoderStatus.OK) { // ตรวจสอบสถานะ ถ้าหากเจอ
                        ...
                        parent.$("#lat_value").val(my_Point.lat());  // เอาค่า latitude พิกัดผลลัพธ์ แสดงใน textbox id=lat_value
                        parent.$("#lon_value").val(my_Point.lng());  // เอาค่า longitude พิกัดผลลัพธ์ แสดงใน textbox id=lon_value 
                        parent.$("#zoom_value").val(map.getZoom()); // เอาขนาด zoom ของแผนที่แสดงใน textbox id=zoom_valu    
                        map.panTo(my_Point); // ให้แผนที่แสดงไปที่ตัว marker    
                        ...
                        parent.$("#namePlace").val("");// กำหนดค่า textbox id=namePlace ให้ว่างสำหรับค้นหาใหม่
                     }
                });
            }else{
                alert('Geocode Error!!!');
        ...
        parent.$("#SearchPlace").click(function(){ // เมื่อคลิกที่ปุ่ม id=SearchPlace ให้ทำงานฟังก์ฃันค้นหาสถานที่
            searchPlace();  // ฟังก์ฃันค้นหาสถานที่
        });
        parent.$("#namePlace").keyup(function(event){ // เมื่อพิมพ์คำค้นหาในกล่องค้นหา
    ...
    function getLocation(lat,long){
        ...
        var url = "https://maps.googleapis.com/maps/api/geocode/json?latlng="+lat+","+long+"&sensor=false";
                ...
                var distric, locality, province;
                    ...
                    parent.$("#locality").val(locality);
                    parent.$("#distric").val(distric);
                    parent.$("#province").val(province);
                    tambol  = locality.split(' ');
                    parent.$("#namePlace").val(tambol[1]+" "+province);
                });
    }
