<div class="row text-center">
  <div class="col-md-12 width-100-per" style="text-align:center; margin-top: -15px;">
      ...
      <thead class="">
          <tr>
              <th width="15%">
                  <button type="button" class="btn btn-secondary primary" onClick="fncPlotView('<?=$dataArea->farmer_list_id?>');" style="float:left; margin-top: 0px; margin-left: -12px; margin-bottom:0px;" >
                      <i class="icon-rewind"></i> Back
                  </button>                    
              </th>
              <th width="*" style="text-align: center;">
              <h5 class=""><?=$dataArea->codename?> [<?=$dataArea->count_all?> แปลง] / [<?=$dataArea->sum_area['rai']?> ไร่]</h5>
              </th>
              <th width="15%">&nbsp;</th>
          </tr>
      </thead>
                    ...
                    <div class="col-md-12">
                        <div class="form-body">
                            <div class="control-group">
                                <label style="width:91.5%; text-align:center; margin-top:10px;">
                                    ...
                                    <fieldset class="form-group position-relative" style="text-align-center">
                                            <input type="text" id="namePlace" class="form-control" placeholder="ค้นหาจังหวัด / อำเภอ" name="name_place" value="<?=$data->name_place?>">
                            ...
                            <div class="control-group">
                                ...
                                <input type="text" class="form-input width-60-per mb-1" name="codename" value="<?=$data->codename?>" required>
                            </div>
                            <div class="control-group">
                            <label class="width-30-per mb-0" style=" text-align:left;">Lat</label>
                                <input type="text" class="form-input width-60-per mb-1" name="lat_value" value="<?=$data->lat_value?>" required>
                            </div>
                            <div class="control-group">
                            <label class="width-30-per mb-0" style=" text-align:left;">Long</label>
                                <input type="text" class="form-input width-60-per mb-1" name="lon_value" value="<?=$data->lon_value?>" required>
                            </div>
                            <div class="control-group">
                            <label class="width-30-per mb-0" style=" text-align:left;">Zoom</label>
                                <input type="text" class="form-input width-60-per mb-1" name="zoom_value" value="<?=$data->zoom_value?>" required>
                            </div>
                            <div class="control-group">
                            <label class="width-30-per" style=" text-align:left;">พื้นที่</label>
                            <input type="text" class="form-input width-20-per mb-1" name="rai" placeholder="ไร่" maxlength="6" value="<?=$data->rai?>" required>
                            <input type="text" class="form-input width-20-per mb-1" name="ngan" placeholder="งาน" maxlength="3" value="<?=$data->ngan?>" required>
                            <input type="text" class="form-input width-20-per mb-1" name="square" placeholder="ตารางวา" maxlength="3" value="<?=$data->square?>" required>
                            </div>
                            <?php 
                            if($data->path_file<>''){
                            ?>
                            <div class="control-group">
                             <a href="<?=_HTTP_HOST_.'/'.$data->path_file?>" target="_bank">view</a>
                             </div>
                            <?php } ?>
                            ...
                            <div class="control-group">
                              ...
                              <input type="text" class="form-input width-60-per mb-1" value="<?=($data->bn=='')?'ไม่มี':''?>">
                            </div>
        ...
        success: function(result) {
              ...
              if(obj.result=='true') {console.log('true');
                    $('#md-error').on('hidden.bs.modal', function (e) {
                        fncPlotView('<?=$dataArea->farmer_list_id?>');
