                ...
                <h4 class="card-title"> <?=$page_name?></h4>
                  <div class="heading-elements form-actions right">
                      <button id="addFGroup" type="button" class="btn btn-primary" onClick="fncLoadContent('<?=$page_starturl?>create');">
                          <i class="icon-database2 white"></i> New Farmer
                      </button>
                  </div>
            </div>
            <div class="table-responsive">
                <table class="table table-striped mb-0" id="tb-farmer-list">
                  <thead class="bg-green bg-lighten-2">
                    <tr>
                      <th class="text-center"  width="6%">ลำดับ</th>
                      <th width="30%">เกษตรกร</th>
                      <th width="20%">กลุ่ม</th>
                      <th width="15%">ประเภทการปลูก</th>
...
<script type="text/javascript">
  var oTable = $('#tb-farmer-list').DataTable({
    /*"language": {
  ...
  //key enter to search
  $('#tb-farmer-list_filter input').unbind();
  $('#tb-farmer-list_filter input').bind('keyup', function(e) {
    if(e.keyCode == 13) {
